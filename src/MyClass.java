public class MyClass {

    //Version v1
    public String string1 = "Bu bir string'dir.";

    //Version v2
    public String string2 = "Bu da baska bir string'dir.";

    //Version v3
    public String string3 = "Bu da cok daha baska bir string'dir.";

    //Techpro branch, version t1
    public String string4 = "Bu techpro branch'inde olusturulmus baska bir string'dir.";

    //Techpro branch, version t2
    public String string5 = "Bu techpro branch'inde olusturulmus baska bir string'dir.";

    //Techpro branch, version v4
    public String string6 = "Bu techpro branch'inde olusturulmus baska bir string'dir.";

    //Techpro branch, version t3
    public String string7 = "Hangi string oldugunun hesabani yapamadim.";


    //Techpro branch, version t4
    public String string8 = "Dar geldi ankara.";

    //Techpro branch, version t5
    public String string9 = "Affet beni aksam ustu";
    //Conflict example, t6
    public String string10 = "Bu aksam  kimse beni ";
    //Conflict example, v7
    public String string11 = "Eski koprunun altinda";


}